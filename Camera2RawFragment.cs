...
namespace Camera2Raw
{
    public class Camera2RawFragment : Fragment, View.IOnClickListener
    {
                                                     ...
                                                     TotalCaptureResult result)
            {
                ...
                lock (mCameraStateLock)
                {
                    ...
                    if (jpegBuilder != null)
                    {
                        ...
                        location = jpegBuilder.GetSaveLocation();
                    }
                    if (rawBuilder != null)
                    {
		...
		class ImageFileHandler : Handler
		{
			...
			public ImageFileHandler(Looper looper, Activity activity) : base(looper)
			{
				Activity = activity;
			}
			...
			public override void HandleMessage(Message msg)
			{
				if (Activity != null)
				{
                    var file = new Java.IO.File((string)msg.Obj);
                    Bitmap myBitmap = BitmapFactory.DecodeFile(file.AbsolutePath);
					Activity.RunOnUiThread(() =>
					{
                        mTextureView.Visibility = ViewStates.Invisible;
		...
		/// A {@link Handler} for showing {@link Toast}s on the UI thread.
		/// </summary>
		static Handler mImageFileHandler;
        ...
        public override void OnViewCreated(View view, Bundle savedInstanceState)
        {
            ...
            image = (ImageView)view.FindViewById(Resource.Id.image);
            ...
            mImageFileHandler = new ImageFileHandler(Looper.MainLooper, Activity);
		...
		static void ShowImage(string text)
		{
			// We show a Toast by sending request message to mMessageHandler. This makes sure that the
			// Toast is shown on the UI thread.
			Message message = Message.Obtain();
			message.Obj = text;
            mImageFileHandler.SendMessage(message);
		}
